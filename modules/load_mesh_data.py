...
def read_fesom_mesh(path, alpha, beta, gamma, read_diag=True): 
	...
	mesh.y2=file_content[:,2]
        mesh.nodes=file_content[:,3]
	mesh.n2d=len(mesh.x2)

	file_content = np.loadtxt(nod3dfile, comments='!', skiprows=1)
	mesh.x3=file_content[:,1]
	mesh.y3=file_content[:,2]
	mesh.n3d=len(mesh.x3)
	...
	zcoord=np.loadtxt(nod3dfile, comments='!', skiprows=1)[:,3]
	...
	#we should rotate the mesh to the geographical coordinates
        (mesh.x3,mesh.y3)=scalar_r2g(mesh.alpha,mesh.beta,mesh.gamma,mesh.x3,mesh.y3)
	#d=mesh.x2[mesh.elem].max(axis=1)-mesh.x3[mesh.elem].min(axis=1)
        print '3d rotation finished'
...
def read_fesom_3d(str_id, months, years, mesh, result_path, runid, ext, how='mean'): 
	...
	while y<=years[1]:
		...
		if how=='mean':
			data3 = data3+f.variables[str_id].data[months,:].mean(axis=0)
		elif how=='max':
			data3 = data3+f.variables[str_id].data[months,:].max(axis=0)
		f.close()
		y=y+1
...
def fesom2depth(depth, data3, mesh): 
	import numpy as np
	data2=np.zeros(shape=(mesh.n2d))
        dind=(abs(np.flipud(mesh.zlevs)-depth)).argmin()
